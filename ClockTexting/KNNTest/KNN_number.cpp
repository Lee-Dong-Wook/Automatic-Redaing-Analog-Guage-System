...
int main()
{
	...
	string image_file = "../trainImage/train_numbers.png";
	Mat  train_image = imread(image_file, 0);		// 영상 로드
	CV_Assert(train_image.data);

	threshold(train_image, train_image, 32, 255, THRESH_BINARY); //CV_THRESH_BINARY
	...
	string demo_file = format("../trainImage/num/%02d.png", no);
