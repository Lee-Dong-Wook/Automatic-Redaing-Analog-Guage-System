...
Mat  preprocessing(Mat image, Mat& morph)
{
	...
	cvtColor(image, gray, COLOR_BGR2GRAY); //CV_BGR2GRAY
...
Mat  correct_plate(Mat input, RotatedRect ro_rect)
{
	...
	warpAffine(input, rot_img, rotmat, input.size(), INTER_CUBIC); //CV_INTER_CUBIC
...
Ptr<ml::SVM> SVM_train(string fn)
{
	...
	svm->setGamma(1);
	svm->setC(1);
	svm->setTermCriteria(TermCriteria(TermCriteria::MAX_ITER, 1000, 0.01)); //CV_TERMCRIT_ITER
...
vector<Mat> make_candidates(Mat img, vector<RotatedRect>& ro_rects)
{
	...
	for (size_t i = 0; i < ro_rects.size();)	{
		...
		if (vertify_plate(ro_rects[i]))
		{
			...
			cvtColor(corr_img, corr_img, COLOR_BGR2GRAY);
